...
struct _tc_iot_mqtt_client {
    ...
    int command_timeout_ms;  /**< MQTT 指令超时配置，根据客户端设备工作环境，实际网络情况进行合理配置。
                                        ...
                                        能及时被超时机制发现，及时重连恢复服务。*/
    int buf_size;  /**< 发送缓存区大小，固定为 TC_IOT_CLIENT_SEND_BUF_SIZE*/
    int readbuf_size;  /**< 接收缓存区大小，固定为 TC_IOT_CLIENT_READ_BUF_SIZE*/
...
int tc_iot_mqtt_client_publish(tc_iot_mqtt_client* client, const char*,
                               tc_iot_mqtt_message*);
int tc_iot_mqtt_client_subscribe_with_results(tc_iot_mqtt_client* client,
                                              const char* topicFilter,
                                              tc_iot_mqtt_qos_e, message_handler,
                                              void * context,
                                              tc_iot_mqtt_suback_data* data);
int tc_iot_mqtt_client_unsubscribe(tc_iot_mqtt_client* client,
                                   const char* topicFilter);
int tc_iot_mqtt_client_disconnect(tc_iot_mqtt_client* client);
