...
int tc_iot_mqtt_client_connect(tc_iot_mqtt_client* c) {
    ...
    if (TC_IOT_SUCCESS == rc) {
        ...
        tc_iot_mqtt_event_handler(c, TC_IOT_MQTT_EVENT_CONNECT_SUCCESS, &rc, NULL);
    } else {
        ...
        tc_iot_mqtt_event_handler(c, TC_IOT_MQTT_EVENT_CONNECT_FAILED, &rc, NULL);
    }
    return rc;
}

int tc_iot_mqtt_event_handler(tc_iot_mqtt_client * c, int event_type, void * data, void * context) {
    tc_iot_event_message msg;
        ...
        if (c->clean_session) {
            for (i = 0; i < TC_IOT_MAX_MESSAGE_HANDLERS; ++i) {
                if ( c->message_handlers[i].topicFilter != NULL) {
                    rc = tc_iot_mqtt_client_subscribe(c,
...
int deliverMessage(tc_iot_mqtt_client* c, MQTTString* topicName,
    ...
    if (left > 0) {
        ...
        tc_iot_mqtt_event_handler(c, TC_IOT_MQTT_EVENT_SERVER_PUB_TOO_BIG, &error_code, NULL);
        return error_code;
    }
...
exit:
    ...
    if (rc == TC_IOT_SUCCESS) {
        tc_iot_mqtt_event_handler(c, TC_IOT_MQTT_EVENT_RECONNECT_SUCCESS, &rc, NULL);
    } else {
        tc_iot_mqtt_event_handler(c, TC_IOT_MQTT_EVENT_RECONNECT_FAILED, &rc, NULL);
    }
    return rc;
}
...
int tc_iot_mqtt_connect_with_results(tc_iot_mqtt_client* c,
    ...
    if (!options) {
        ...
        tc_iot_mqtt_init_conn_data(options);
    }
...
exit:
    ...
    if (rc == TC_IOT_SUCCESS) {
        tc_iot_mqtt_event_handler(c, TC_IOT_MQTT_EVENT_SUB_SUCCESS, &rc, NULL);
    } else {
        tc_iot_mqtt_event_handler(c, TC_IOT_MQTT_EVENT_SUB_FAILED, &rc, NULL);
    }
    return rc;
}

int tc_iot_mqtt_client_subscribe(tc_iot_mqtt_client* c, const char* topicFilter,
                          tc_iot_mqtt_qos_e qos,
                          message_handler message_handler,
                          void * context) {
    tc_iot_mqtt_suback_data data;
...
int tc_iot_mqtt_client_unsubscribe(tc_iot_mqtt_client* c, const char* topicFilter) {
...
exit:
    ...
    if (rc == TC_IOT_SUCCESS) {
        tc_iot_mqtt_event_handler(c, TC_IOT_MQTT_EVENT_UNSUB_SUCCESS, &rc, NULL);
    } else {
        tc_iot_mqtt_event_handler(c, TC_IOT_MQTT_EVENT_UNSUB_FAILED, &rc, NULL);
    }
    return rc;
}

int tc_iot_mqtt_client_publish(tc_iot_mqtt_client* c, const char* topicName,
                        tc_iot_mqtt_message* message) {
    int rc = TC_IOT_FAILURE;
    tc_iot_timer timer;
    MQTTString topic = MQTTString_initializer;
...
exit:
    ...
    if (rc == TC_IOT_SUCCESS) {
        tc_iot_mqtt_event_handler(c, TC_IOT_MQTT_EVENT_PUB_SUCCESS, &rc, NULL);
    } else {
        tc_iot_mqtt_event_handler(c, TC_IOT_MQTT_EVENT_PUB_FAILED, &rc, NULL);
    }
    return rc;
}
...
int tc_iot_mqtt_client_internal_disconnect(tc_iot_mqtt_client* c, int r) {
    ...
    tc_iot_mqtt_set_state(c, CLIENT_INTIALIAZED);
    tc_iot_mqtt_event_handler(c, TC_IOT_MQTT_EVENT_DISCONNECTED, &r, NULL);
    return rc;
}
...
int tc_iot_mqtt_refresh_iothub_dynamic_sign(long timestamp, long nonce, tc_iot_device_info* p_device_info, long reserve) {
    ...
    secret_len = tc_iot_base64_decode(device_secret, strlen(device_secret), raw_secret, sizeof(raw_secret));
    ...
    tc_iot_hal_snprintf(p_device_info->username, sizeof(p_device_info->username), 
            "%s;%s;%s;%u", p_device_info->client_id, QCLOUD_IOT_DEVICE_SDK_APPID, conn_id, (unsigned int)timestamp+300);
    ...
    /* } else { */
        ...
        ret = tc_iot_calc_sign( sha256_digest, sizeof(sha256_digest), raw_secret, secret_len, "%s", username);
        tc_iot_hal_snprintf(password,
                            sizeof(p_device_info->password),
                            "%s;hmacsha256",
                            /* hex_buf */
